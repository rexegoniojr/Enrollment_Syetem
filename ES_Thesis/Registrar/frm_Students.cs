        ...
        private void lstStudents_DoubleClick(object sender, EventArgs e)
        {
            ...
            else
            {
                if (String.IsNullOrEmpty(Global.stud_info) || String.IsNullOrWhiteSpace(Global.stud_info))
                {
                    ...
                    if (stdINFOs == null || stdINFOs.IsDisposed)
                    {
                        stdINFOs = new frm_student_info();
                        stdINFOs.Show();
                    }
                    else
                    {
                        stdINFOs.BringToFront();
                    }
                }
            }
        }

        private void txtStudent_Search_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                lstStudents.Items.Clear();
                DBMysql.con.Close();
                DBMysql.cmd = new MySqlCommand(String.Format("select * from tbl_Personal_Info where LN like '%{0}%' or FN like '%{0}%' or StudNum like '%{0}%'"+
                "order by LN asc, Program asc",txtStudent_Search.Text), DBMysql.con);
                DBMysql.con.Open();
                DBMysql.reader = DBMysql.cmd.ExecuteReader();
                while (DBMysql.reader.Read())
                {
